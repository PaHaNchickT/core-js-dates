...
function getCountDaysInMonth(month, year) {
  const end = new Date(`${year}-${(month + 1).toString().padStart(2, 0)}-01`);
  return new Date(end.setDate(end.getDate() - 1)).getDate();
}
...
function isDateInPeriod(date, period) {
  ...
  if (check - start >= 0 && end - check >= 0) return true;
  return false;
}
...
function formatDate(date) {
  ...
  let h = out.getUTCHours();
  let mode = 'AM';
  if (out.getUTCHours() >= 12) {
    mode = 'PM';
  }
  if (out.getUTCHours() > 12) {
    h = out.getUTCHours() - 12;
  }

  return `${out.getMonth() + 1}/${out.getUTCDate()}/${out.getFullYear()}, ${h}:${out.getMinutes().toString().padStart(2, 0)}:${out.getSeconds().toString().padStart(2, 0)} ${mode}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const out = [];
  const end = date.getMonth();
  while (date.getMonth() === end) {
...
function getWeekNumberByDate(date) {
  ...
  if (start.getDay() - end.getDay() >= 0) counter += 1;
  return counter;
}
...
function isLeapYear(date) {
  if (new Date(`${date.getFullYear()}-02-29`).getMonth() === 1) return true;
  return false;
}
